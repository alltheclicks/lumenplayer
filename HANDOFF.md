...
- Current guardrails on PR #224:
  ...
  - `scripts/release/validate-no-media-evidence-scan-artifact.mjs` now verifies the referenced raw Chrome Network/CDP evidence paths are repo-relative, untracked by git, and hash-match the tracked redacted artifact when the local raw files are present
