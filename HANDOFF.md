...
- **NIJE URAĐENO (čeka pristup produkciji):** generisanje `PLAYER_SSO_SECRET` + upis u `/opt/lumen/proxy/proxy.env` i exyu.tv `.env`; scp nginx conf + reload; `scripts/deploy/deploy-vps.sh` sa exyu worktree-a; exyu.tv deploy + PM2 restart. Redosled i verifikaciona lista u planu `/Users/filip/.claude/plans/kao-sto-vidis-doslo-tender-metcalfe.md`.
- **Rizici:** (1) exyu.tv build blokiran RSS feedom (gore); (2) qaf035 release gate na ovoj grani možda traži artefakte sa catchup grane — fallback `LUMEN_RELEASE_GATE=current`; (3) `docs/WEB-PLAYER-V1-PILOT.md` u exyu repou i dalje tvrdi "nije SSO" — ažurirati posle deploy-a.
