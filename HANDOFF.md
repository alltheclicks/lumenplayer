...
- `playerAnalytics.ts`: batch na 5s/25 događaja/visibility/unload beacon, bounded offline retry, session/device/foreground/active/playback metrike, route + semantički UI klikovi sa normalizovanim koordinatama, viewport klasom i input metodom, search tekst, media QoE (first frame, buffering, switch latency, rezolucija, dropped/total frames), postojeći observability playback/catch-up/cast događaji, global error/unhandled rejection i React ErrorBoundary crash fingerprint. Kumulativne metrike preživljavaju reload u istoj tab sesiji.
...
- `panel-new` sada ima 7/30/90 prikaz, striktni playback funnel, semantički heatmap kontrola, koordinatni 10×6 heatmap po viewportu, distribuciju strukturisanih grešaka i crash fingerprint grupe. Korelacija greške broji najviše jedan ishod po kliku u prozoru od 30 sekundi.
- Novi tab sa postojećim Xtream kredencijalima obnavlja samo opaque analytics binding preko autentifikovanog `GET /player-analytics/config`; endpoint ne vraća XUI podatke, secret ili upstream URL. Bez validnog cookie-ja vraća 401, a pogrešan Origin 403.
- Validacija i produkcija: lint ✅, typecheck ✅, 418/418 unit ✅, production audit 0 poznatih ranjivosti ✅, production build ✅. EXYU backend release `20260712164625` (`5d73414`, insights od `40ee6d0`) i Lumen release `20260712T144323Z-2d2cad6` su live. Realan SSO, playback, feedback, feedback-before-replay kompatibilnost, sintetički crash, rrweb download i koordinatni click ingest potvrđeni su kroz prijavljenu Chrome sesiju i `panel-new`; novi feedback i crash imaju direktan replay ID. Posle deploy-a nema novih `22P02`/`23503` grešaka. Cloudflare service-worker cache je `no-store`/`BYPASS`, dok verzionisani `/assets/*` ostaju immutable.
...
- **Login strana više ne prikazuje Xtream hostname** (`0dc4ef6`): uklonjen "Server: gw.castcdn.net" blok iz `Login.tsx`; admin alert za nekonfigurisan server ostaje.
- **Bešavni SSO exyu.tv → player** (`5d5c4a6`): tok = exyu.tv `GET /api/player-sso` (Supabase cookie auth + `user_subscription.is_active` + XUI lozinka iz MySQL) → 302 na `player.exyu.tv/sso#token=<t>` (AES-256-GCM, TTL 60s, deljeni `PLAYER_SSO_SECRET` 64-hex) → SPA `/sso` (`SsoLanding.tsx`) skine fragment, POST `/sso/exchange` → Fastify proxy (`apps/proxy/src/sso.ts`) dekriptuje i vrati kredencijale → normalan `authenticate()` + `saveXtreamCredentials` → `/player`. Proxy ruta: inertna bez secreta (404), CORS origin gate, per-klijent rate limit (pravi IP iz XFF/CF headera), jti replay cache, nikad ne loguje token/kredencijale. nginx vhost dobio `location = /sso/exchange` (limit_req zona `lumen_sso`). Vite dev proxy mapira `/sso/exchange` → 127.0.0.1:8788. 18 novih unit testova (`sso.test.ts`); typecheck/lint/build/398 unit ✅.
- **exyu.tv strana (repo `/Users/filip/Documents/exyu-tv-nextjs`, NECOMMIT-OVANO na `main`):** novo `src/lib/player-sso.ts` + `src/app/api/player-sso/route.ts` (Node runtime, pilot gating preko `WEB_PLAYER_PILOT_EMAILS` dok je postavljen — prazan = javna beta); `player/page.tsx` CTA sada vodi na `/api/player-sso`; `PlayerCTAs.tsx` tekst ažuriran. tsc + eslint ✅. **`next build` PADA na PRE-POSTOJEĆOJ grešci**: `/vesti/[slug]` prerender vuče Soro RSS feed koji trenutno vraća neispravan XML ("CDATA is not closed") — nevezano za SSO, ali blokira deploy sajta dok feed ne proradi ili se parser ne ogradi.
- **DEPLOY ZAVRŠEN 12.7. — SVE LIVE I VERIFIKOVANO:**
  - Player VPS: nginx snippet ažuriran (`/sso/exchange` + `lumen_sso` zona u conf.d, root → `/var/www/lumen-current` symlink; backup `player-app.conf.bak-20260712`), `PLAYER_SSO_SECRET` u `/opt/lumen/proxy/proxy.env` (CORS origins sada `https://player.exyu.tv,http://151.241.151.105`); **prvi versioned release kroz novi `deploy-vps.sh` aktiviran: `20260711T232915Z-1df8eff`** (proxy migriran u `/opt/lumen/proxy-releases` + `proxy-bootstrap`). Prod smoke: garbage 400, validan token 200, replay 401, tuđi origin 403, bundle bez "Server:" labele.
  - exyu.tv: `PLAYER_SSO_SECRET` u `shared/.env` (backup `.env.bak-20260712`), release `20260712014225`. Commiti `ca90b8a` (SSO ruta+CTA), `148f6d1` (prazna pilot lista = javna beta, CTA vide svi aktivni), `08a…` fix redirect origina (`getTrustedSiteOrigin` — `req.url` iza nginx-a nosi localhost). `GET /api/player-sso` bez sesije → 302 `https://exyu.tv/nalog?next=/player`, no-store ✅.
  - qaf035 gate: brand guard-ovi usklađeni sa EXYU brendingom (`1df8eff`), deploy išao sa `LUMEN_RELEASE_GATE=current`.
- **Napomene:** (1) Soro RSS povremeno servira neispravan XML i tada obara `next build` `/vesti/[slug]` prerender — ograditi parser (nije rađeno); (2) exyu-tv-nextjs repo NEMA git remote (commiti samo lokalni); (3) `WEB_PLAYER_PILOT_EMAILS` nije postavljen na serveru = javna beta aktivna za sve pretplatnike; (4) `docs/WEB-PLAYER-V1-PILOT.md` u exyu repou i dalje tvrdi "nije SSO" — zastarelo; (5) pun E2E klik-test sa realnim nalogom ostavljen vlasniku.
...
- **Tokeni (`apps/web/src/index.css`):** cela paleta pomerena na navy-tinted dark (hue 223–225), primary 217→221, **`--catchup` sada ljubičast (265 84% 66%)** — razdvojen od primary; `--success` ublažen; `--radius` 0.75→0.85rem; novi glass tokeni (`--overlay/--glass/--glass-border/--focus-glow/--elevated/--scrim`); **motion sistem** (`--motion-instant..slower/stagger/tv-*`, `--ease-out-soft/in-out-soft/exit/standard/spring` + `@supports linear()` spring aproksimacija). Sve mapirano u `tailwind.config.js` (success/live/catchup/glass boje, `duration-*`, `ease-*`, `animate-toast-in`).
- **Globalni motion sloj (`index.css` @layer base):** tranzicije svih interaktivnih elemenata preko motion tokena, `button:active` press, **layered focus-visible glow ring (TV/D-pad ready)** umesto shadcn ring-2 (uklonjen iz button/input varijanti), `prefers-reduced-motion` kill. Keyframes `lp-pulse-dot`; klase `.lp-live-dot`, `.lp-player-overlay`.
- **Komponente:** button varijante hover lift+glow senke; input hover elevate/focus border; Card hover lift (svi Card-ovi!); toast spring enter (`animate-toast-in`) + hover lift; `.badge-live`/`.badge-catchup` sada `--live`/`--catchup` tokeni (UNAZAD badge više nije hardkodovan orange).
- **Player (`PlayerControls.tsx`):** scrim `.lp-player-overlay` (4-stop overlay gradijent) na obe donje trake + top bar na overlay tokene; livebar 6px→9px hover, gradient fill primary→focus-glow + glow, beli thumb 15px sa halo; glass klasteri (`bg-glass/50`, `backdrop-blur-[18px]`, saturate, inset highlight); dugmad u klasterima `hover:bg-foreground/10`; UŽIVO badge sa pulsirajućom tačkom.
- **Emerald sweep:** ChannelList + Player.tsx (TV Unazad panel) emerald→`catchup` token (opacity mapiranje po dizajnu); Settings emerald→`success` (semantika statusa); ChannelList logo pločica gradijent surface-2→card, favorite hover→`live`. Ostalo hardkodovano: dekorativni gradijenti VodCategories/SeriesCategories (nije u scope-u pull-a).
- **NIJE povučeno u kod:** `motion/*` demo kartice i `tv-motion/*` (10-foot spec za T5 fazu — ostaju u design projektu kao specifikacija); "Tweaks panel" iz player-controls kartice (alat samog design editora).
- **Generator:** `build-bundle.mjs` — `extractCva` sada podnosi komentare u `cva(`, tokens.css nosi i `@supports` blok.
- **Validacija:** typecheck/lint/build ✅, 303/303 unit ✅, vizuelni smoke login ✅ (nova paleta renderuje).
- **Dovršeno u nastavku sesije:** aktivan kanal u listi dobio animirani ekvilajzer (`.lp-eq`, samo scaleY) + hover glow/scale redova po motion-lists spec-u; snapshot skripta sada podržava `DESIGN_SNAPSHOT_STORAGE_STATE` (authed capture preko sačuvanog Playwright storage-state-a, bez rukovanja lozinkom — korišćen QA-GATE artefakt); svih 13 authed ekrana ponovo snimljeno sa novim dizajnom i **ceo bundle (26 fajlova) push-ovan u claude.ai/design → remote je sada kanonski baseline**. `_ds_manifest.json` NIJE push-ovan (održava ga design app; 33 kartice uklj. Motion/TV Motion — provereno posle push-a).
- **Next:** (1) vizuelni pregled player/EPG/Settings u browseru + odluka o commit-u redizajna; (2) C0-a/b/c iz master plana i dalje čekaju.

## Session 2026-07-06 — Multi-platform audit + MASTER PLAN za V1→TV ekspanziju

- **Novi krovni dokument: `docs/PLAN-MULTIPLATFORM-EXECUTION.md`** — obavezno čitanje za sve buduće agente. Sadrži: arhitektonske invarijante nezavisnosti segmenata (I-1..I-8, hard gate za PR-ove), faze V1-završnica → P4 platformizacija → T5 Tizen/webOS → A6 Android TV → Apple TV, taskove sa ID-jevima (C0.x, P4.x, T5.x, A6.x), otvorene vlasničke odluke (O-1..O-7) i graf zavisnosti. `RELEASE-PLAN-MVP-BETA-FINAL.md` ostaje operativni tracker za B2.x/F3.x.
- **Multi-agent audit (35 agenata, adversarijalno verifikovano):** arhitektura obrnuta (~25.3k non-test linija u `apps/web` vs ~2.9k u svim paketima; ~7.2k linija platform-agnostične logike zarobljeno u app-u — ekstrakcija mehanička, ~15 DI dodirnih tačaka); `RendererAdapter` 0 implementacija (Cast=React hook, AirPlay=inline webkit, CAF receiver funkcionalno mrtav); nema cross-device session transporta (BroadcastChannel+localStorage); `session-core` JESTE implementiran (CLAUDE.md/BACKLOG zastareli); `HlsPlayerAdapter` čist i preseljiv u paket skoro verbatim. Pun nalaz u memoriji `multi-platform-readiness-audit`.
- **Dijagnoza vlasnikovog "stuca" na catch-up rebase putu:** frozen-frame maskiranje fizičke GOP rupe 0.7–2s na svakoj minutnoj granici (~41/101/161s media time; video stane, audio teče, mali skok) — fizički pod, frejmovi ne postoje ni na serveru (`dump_extra` ih NE vraća). Peglanje = taskovi C0-a (`stretchShortVideoTrack`), C0-b (`maxBufferHole`~2.0 za rebase), C0-c (boundary telemetrija: `videoHoleMs` + predviđena boundary vremena + `catchup.stall`). Detalji u memoriji `catchup-client-pts-rebase`.
- **Next:** C0-a/b/c (malo koda, odmah merljivo) → C0-d vlasnikov retest (obrisati `lumen:catchup-client-rebase:v1`!) → O-1 push/prod odluka; paralelno B2.1-a po release planu.

## Session 2026-07-02 (c) — RTS 1 decode greška na granici: zasečen poslednji video PES fajla (VideoToolbox BadData)

- **Prijava vlasnika (Brave, RTS 1 Dnevnik 19:30):** na 0:58 "load pa skip 4–6s"; posle seek-a na 4:03 išlo 15s pa opet load+skip. Repro kroz aplikaciju (Playwright, isti program): na **prvoj minutnoj granici** (media ~41s) element baca `MEDIA_ERR_DECODE`; hvatanjem pune poruke: **`VideoToolbox kVTVideoDecoderBadDataErr (-12909)`** — deterministički, 100% repro na toj granici. Legacy odgovor aplikacije = `runtime_decode_skip` (+15s reload) → to je korisnikov "skip". hls.js element error uopšte ne vidi (ne sluša ga) pa njegov recovery nikad ne krene.
- **KOREN:** XUI writer seče arhivske fajlove na wall-clock **usred video frejma** → poslednji video PES svakog minuta je ZASEČEN. Na legacy putu discontinuity reset je to (uglavnom) gutao (uz povremene decode greške — zato ceo `runtime_decode_skip` aparat i postoji, max 8×15s!); na kontinualnom rebase putu demuxer flush-uje polu-frejm u SourceBuffer i striktni hardverski dekoder (macOS VT) ga odbija. NOVA S je prolazila (tolerantniji sadržaj/seča), RTS 1 ne.
- **FIX 1 (`mpegTsPtsRebase.ts`):** scan pamti pakete poslednjeg video PES-a u fajlu; rewrite ih pretvara u null pakete (PID 0x1FFF, dužina ista) — gubitak ≤1 frejm/min koji ionako stoji ispred video rupe na granici. Novo polje `record.droppedTailPackets`. **Live potvrda:** ista RTS 1 granica sada prolazi bez ijedne greške; granice na ~41/~101/~161 čiste, 1.000× rate, buffered kontinualan, 0 retry-ja.
- **FIX 2 (`HlsPlayerAdapter.ts`, odbrana u dubinu):** element `MEDIA_ERR_DECODE` u catch-up + hls modu sada prvo ide kroz in-place `hls.recoverMediaError()` budžet (4, cooldown 5s reset kroz `resetBufferingRecovery`) umesto pravo u session skip; plus **enforcement resume pozicije** 12s posle recovery-ja (session-ov re-attach startup zna da povuče playhead na početak — viđeno u testu: 42→1.2). Tek posle budžeta ide `MEDIA_ELEMENT_3` → postojeći skip lanac. +3 adapter testa.
- **Validacija:** typecheck/lint/build ✅, 293/293 unit ✅ (17 rebase + 45 adapter), live RTS 1 kroz celu aplikaciju ✅.
- **Napomena za seek UX:** UI seek pravi novi prozor (`start=HH-MM`) → seg=0 tog prozora na stock XUI gubi ~18s glave (fseek 30%) + pun download 45MB minute pre prvog frejma → seek latencija realno 5–15s; watchdog (10s, 1 retry) na sporim vezama ume da odbaci prvi pokušaj. Kandidat za tuning (veći watchdog za rebase izvore / preload), NIJE rađeno u ovoj sesiji.
- **Next:** vlasnikov retest RTS 1 + PINK + NOVA S (obrisati `lumen:catchup-client-rebase:v1` markere); zatim odluka push/prod.

## Session 2026-07-02 (b) — REAL-XUI validacija rebase-a: seg=0 alignment fix + potvrda na živom kanalu

- **Povod:** vlasnik testirao (a)-verziju na dev serveru — NOVA S catch-up nije ni krenuo (spinner „Pokrećem snimak", bafer 19s), PINK pustio ali potpuno desinhronizovan. Debug sa realnim segmentima (fica @ gw.castcdn.net → edge6, NOVA S=381, PINK=105) našao KOREN: **stock XUI `seg=0` (fseek 30%) počinje USRED TS paketa** → `parseTs` promaši sync grid → rebase vrati `not-ts` fatal → cela sesija padne na SIROVI legacy m3u8 (edge6 playlist NEMA `#EXT-X-DISCONTINUITY` tagove!) → hls.js guta per-file PTS resete bez ikakvog signala = zaglavljen start / akumulirani A/V drift. Oba simptoma su isti uzrok.
- **Fix (commit na ovoj grani):** `findTsAlignment` u `mpegTsPtsRebase.ts` — probe sync bajta sa 188-periodičnošću (5 paketa), skidanje vodećeg/pratećeg parcijalnog paketa u kopiju, anomalije `leading/trailing-partial-packet`; offseti zaokruženi na cele PTS tikove. +2 unit testa (16/16).
- **NOVO SAZNANJE — premisa „sadržaj je 100% kontinualan" je NETAČNA za video:** brojanje dekodiranih frejmova preko 2 realna minuta (PINK): audio TAČNO 120.00s, video 117.32s. **Na svakoj minutnoj granici arhivi fizički fali do jednog GOP-a videa (~0.7–2s; GOP=2s)** — writer završi fajl na keyframe-u, sledeći počne na SLEDEĆEM IDR-u; međuframe-ovi ne postoje nigde (byte-compare preko granice: 0 duplikata, video i audio susedni bez preklapanja). Audio je kompletan i savršen za chaining. hls.js OVO VEĆ MASKIRA kad su fragmenti contiguous: `mp4-remuxer` na `foundHole` produži PRVI frejm sledećeg fragmenta unazad (frozen-frame preko rupe, bez stall-a) — isto što TiviMate radi. Dakle: audio-chained offseti su ispravni, video rupe premošćuje postojeći hls.js mehanizam, `AVC hole between fragments` warn na granicama je OČEKIVAN (= maskiranje radi), NIJE bug signal.
- **Validacija (sve na REALNIM podacima):** (1) modul: NOVA S + PINK seg0–2 kroz node — svi `rebased`, chained delta=0, seg0 anomalija uredno zabeležena; (2) harness (Chromium MSE): realni segmenti + playlist bez discontinuity — 150s preko obe granice, 0 stall-ova na granicama, bafer JEDAN kontinualan opseg, i seek scenario čist; (3) **CELA APLIKACIJA na localhost:8080 protiv živog edge6**: NOVA S „KVIZ TOTALNI OBRT" (isti program koji vlasniku nije radio) — start OK (TTFRF 2.6–4s), prelazak preko OBE minutne granice sa napredovanjem tačno 1.000s/s i 0 `waiting` eventova, buffered kontinualan [1.4, 224+], `catchup.rebase` summary telemetrija emitovana, NULA fallback eventova; extrapolated offseti potvrđeni (skok na indeks 6 → region tačno pozicioniran uz nearest-record anchoring). typecheck/lint/build/289 unit ✅.
- **Napomene:** raw `video.currentTime` seek van bafera obara app-ov seek watchdog (10s, 1 retry) na restart — UI seek ide kroz setSource reload sa svežom rebase sesijom pa nije pogođen; ali 45MB minutni segmenti + `progressive:false` znače da je seek latencija = pun download segmenta (na sporijim vezama watchdog retry realan — kandidat za tuning). A/V sync na PINK-u proveriti okom (programski nemerljivo), očekivano rešeno jer rebase sada stvarno radi umesto sirovog fallback-a.
- **Next:** (1) vlasnik: ručni test NOVA S + PINK na dev serveru (očisti localStorage `lumen:catchup-client-rebase:v1` ili sačekaj 30min TTL od starih markera!); (2) odluka o push-u grane + prod flag-u; (3) kandidati za doradu: seek watchdog tuning za 45MB segmente, streaming rewriter (faza 2).

## Session 2026-07-02 — KLIJENTSKI catch-up fix: PTS rebase u browseru (bez servera, bez XUI izmena)

- **Grana:** `final-road/catchup-client-pts-rebase` (od `EX-live-refocus-emptysrc`). Flag: `VITE_CATCHUP_CLIENT_REBASE=1` (uključen u dev `.env`, dokumentovan u `.env.production.example`, NIJE uključen za prod).
- **Ideja koja obara „klijent NE MOŽE rešiti" (13.6.):** sadržaj minutnih fajlova JESTE kontinualan — samo XUI archive writer resetuje PTS po fajlu (Uzrok C). Klijent prepiše PES PTS/DTS (+PCR) **u mestu** (5-bajtna polja, bez repaketizacije) sa per-segment offsetom koji lepi sve fajlove u JEDAN kontinualan timeline, i skine `#EXT-X-DISCONTINUITY` iz playliste → hls.js/MSE dekoder nikad ne resetuje → nema splice-a, nema mucanja. Bonus: bez discontinuity reseta SPS/PPS i referentni frejmovi prežive preko granica fajlova → mid-GOP počeci (Uzrok A) dekodabilni na NEsaniranim serverima za sekvencijalno gledanje.
- **Novi kod:** `apps/web/src/adapters/mpegTsPtsRebase.ts` (čist modul: scan → offset memo/chained/extrapolated/grid → in-place rewrite; anomaly ladder → `failed`; 2s joint prag; 33-bit wrap; overlap trim → null paketi 0x1FFF) + 14 unit testova. `HlsPlayerAdapter`: `createCatchUpRebaseFragmentLoader` + `createDiscontinuityStrippingPlaylistLoader`, `progressive` isključen uz rebase (onProgress zaobilazi transform), fatal → in-place reload BEZ rebase-a (resume sa currentTime) + `onCatchUpRebaseFallback`/`onCatchUpRebaseSummary`. Gating: `catchupSource.ts` (rebase ispred shadow pokušaja; shadow/gateway/transport-plan lanac netaknut iza), compat-cache `catchupClientRebaseCompat.ts` (`lumen:catchup-client-rebase:v1`, TTL 30min, per-streamId failure). Telemetrija: `catchup.rebase` (summary/fallback) u `VideoPlayer.tsx`; TTFRF meri novi put automatski.
- **Validacija:** typecheck/lint/build ✅; 287/287 unit ✅; **sintetički Playwright harness** (Chromium MSE + hls.js 1.6.15 + PRAVI rebase modul, 3×60s ffmpeg fajla sa per-file PTS resetom i A/V skew-om kao XUI): 150s preko 2 minutne granice — **0 stall-ova, 0 media errora, boundaryDelta=0, chained offseti tačni**; scenario sa seek-om isto čist. Harness u scratchpad-u (nije u repo-u).
- **NIJE validirano:** protiv realnog XUI catch-up strima (nema Playwright storage-state/kredencijala u sesiji). Sledeći korak = repro recept iz 13.6. (`fica`, PRVA #109) sa flagom uključenim; poređenje TTFRF + stall count vs flag off; hls.js warnovi `overlapping/Injecting/Dropped` posle prvog fragmenta moraju biti 0.
- **Rizici:** `progressive:false` može regresirati TTFRF (meri se postojećim eventom; faza 2 = streaming rewriter u onProgress); MP2 kanali i dalje bez zvuka (rebase ih odbija → fallback, poseban task M1.6); prvi ~18s programa na stock XUI i dalje fizički fali (seg=0 fseek — server šalje od 30%).
- **Next:** (1) runtime validacija na realnom kanalu (gate metrika); (2) odluka o uključivanju flaga na prod; (3) opciono faza 2: streaming rewriter + wasm MP2→AAC.

## Session 2026-06-13 — RUNTIME REPRO catch-up mucanja (Playwright/fica): klijent NE MOŽE rešiti, čeka server

- **Pitanje vlasnika:** „je l' sve na produkciji ili nisi push-ovao fix za mucanje?" → **NIJE.** Grana `final-road/EX-live-refocus-emptysrc` je **ahead 3** od origin-a (`4d4a8fd` + `e0a491a` + `c6a6383`), sve LOKALNO. Produkcija (VPS 151.241.151.105) nema ništa od ovoga; nije ni push-ovano na origin, ni merge-ovano u main, ni redeploy-ovano. (Napomena: prošli HANDOFF kaže da je `f6a7276` deploy-ovan na prod — to stoji; noviji catch-up playhead fix-evi `4d4a8fd`/`e0a491a` su POSLE tog deploy-a i NISU na prod.)
- **Uživo reprodukovano mucanje** (Playwright MCP, vlasnik dao test nalog za ovu svrhu): login `[REDACTED]`/`[REDACTED]` na `gw.castcdn.net`, kanal **PRVA (lista #6)**, catch-up program „06:45 Plodovi" (`data-catchup-start-ms=1781325900000`). Catch-up servira **streamId 109** (PRVA live = 1526 daje 403=mrtav, nebitno za catch-up).
- **Izmereno na `<video>` (sample 500ms / 12s) — DEFINITIVAN merljiv dokaz:**
  - `video.buffered` ZAGLAVLJEN na **[0, 0.47]** — u bafer uđe samo **0.47s** iz celog 60s segmenta. Ostatak = mrtvi P-frejmovi koje hls.js remukser baci (nemaju SPS/PPS).
  - `currentTime` osciluje 0 → ~0.4 → 0 (playhead se POMERA, NIJE zaglavljen na čistoj 0 → **fix `e0a491a` RADI kako je zamišljen**; bundle JESTE u browseru — `startOnSegmentBoundary`/`isCatchUpFromProgramStart`/`HLS_CATCHUP_START_FROM_BEGINNING_POSITION_SECONDS` potvrđeni u serviranom modulu), ali bafer premali da održi reprodukciju → reset.
  - `readyState` 4 do 2 do 0, `buffered` [] do [0,0.47] u krug. Telemetrija: ~75s `MEDIA_ERROR` petlja → `catchup.retry streamId=109` → `PLAYBACK_START_FAILED`. = „muca 20s" koje korisnik vidi.
  - **LIVE radi savršeno** (readyState 4, buffered 30-80s). Problem je STRIKTNO catch-up arhiva.
- **ZAKLJUČAK:** klijentski fix NE MOŽE ovo rešiti — podaci fizički nisu u baferu (remukser baci frejmove bez SPS/PPS). `e0a491a` JESTE ispravan i POSTAJE dovoljan ČIM provider primeni `-bsf:v dump_extra` na snimanju (tad head-skip radi i preko granica segmenata, bez dalje izmene klijenta). Dok server ne legne, mucanje ostaje deploy ili ne. **Pravi blocker = server** (`tools/archive.php` ionCube, XUI dev, spec `docs/XUI-CATCHUP-SPS-PPS-FIX-SPEC.md`).
- **Predlog (čeka vlasnika — NIJE izvršeno):** push grane na origin je bezbedan (live+scrub netaknuti, 42/42 adapter + 357 unit + typecheck/lint/build ✅) i koristan; NE merge-ovati u main / NE redeploy-ovati prod dok server fix ne legne. Vlasnik na kraju tražio da se sve zapiše u memoriju pre odluke o push-u.
- **Next:** (1) vlasnik → XUI dev: aktivirati `dump_extra` na recording floti; (2) po želji push grane na origin; (3) posle server fix-a: ponoviti isti Playwright repro (očekivano `buffered` ide do punih 60s, nema petlje). Repro recept u memoriji `catchup-seg0-no-sps-pps.md`.
...
- Current guardrails on PR #224:
  ...
  - `pnpm release:perf-evidence:validate` now validates `artifacts/release/performance/qaf035-performance-evidence-20260603.json`, so performance readiness uses a concrete QAF artifact instead of only the template
  ...
  - `scripts/release/validate-beta-closure-plan.mjs` now validates `artifacts/release/readiness/qaf035-beta-closure-plan-20260603.json`, mapping each open readiness blocker to owner placeholders, gate ids, artifact refs, validation commands, completion criteria, and the no-media invariant for the next dev/operator step
  - `artifacts/release/readiness/qaf035-go-no-go-20260603.json`, `artifacts/release/smoke/qaf035-smoke-regression-matrix-20260603.json`, and `artifacts/release/design/qaf035-design-parity-20260603.json` now replace the last template-only readiness refs for QAF-035; final signoff remains pending, but link validation now audits concrete artifacts
  - `scripts/release/validate-go-no-go.mjs` and `scripts/release/validate-smoke-regression-matrix.mjs` now reject passing catch-up no-media checks unless evidence cites `release:no-media-evidence:scan` plus a tracked redacted no-media scan artifact that validates
  - `scripts/release/validate-design-parity-evidence.mjs` now requires existing rendered desktop/mobile reference and Lumen capture files in `--require-final`, so design parity cannot be finalized from string refs alone
  - `pnpm release:readiness:validate`, `release:provider-owner:validate`, `release:beta-capacity:validate`, `release:runtime-media:validate`, `release:manual-device-qa:validate`, and `release:beta-ops:validate` now validate the concrete QAF-035 artifacts instead of generic templates, so local operator checks report the same open-blocker state as PR #224 and CI
  - `scripts/release/validate-no-media-evidence-scan-artifact.mjs` now verifies the referenced raw Chrome Network/CDP evidence paths are repo-relative, ignored by git, untracked by git, and hash-match the tracked redacted artifact when the local raw files are present
