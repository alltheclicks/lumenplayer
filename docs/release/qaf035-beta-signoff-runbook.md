...
The final aggregate also directly runs final-mode validation for the concrete go/no-go, smoke/regression, compatibility-matrix, and design-parity artifacts; those artifacts must be complete, approved, and backed by rendered or scanned evidence before the final command can pass. Compatibility final validation uses `--require-matrix artifacts/release/smoke/qaf035-smoke-regression-matrix-20260603.json` and `--require-targets scripts/release/v1-compatibility-targets.template.json`, so the matrix cannot pass after dropping or inventing smoke/regression cases, dropping required beta target profiles, changing a required target id to a narrower platform/device/browser/tag profile, or attaching case results whose tags no longer match the required target profile.
...
Each open closure item must include a direct, gate-specific final proof command for every linked `gateIds` value. `pnpm release:qaf035:final` is the aggregate release rollup, not a substitute for the concrete artifact validator with `--require-final`. Every `artifacts/release/...` proof artifact used by a final proof command must also be listed in that closure item's `artifactRefs`. Non-final validation proves current partial state only; direct final-mode validation is the command that proves the blocker is genuinely closed.
