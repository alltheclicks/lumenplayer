...
The final aggregate also directly runs final-mode validation for the concrete go/no-go, smoke/regression, and design-parity artifacts; those artifacts must be complete, approved, and backed by rendered or scanned evidence before the final command can pass.
