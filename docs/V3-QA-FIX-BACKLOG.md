...
5. Release matrix guard added for beta readiness:
   ...
   - `scripts/release/validate-release-secret-hygiene.mjs` now scans release-facing evidence/docs/templates plus `HANDOFF.md`, `BACKLOG.md`, and this V3 current-truth backlog for non-redacted provider credentials, credential query parameters, env secret assignments, and basic-auth URLs.
   ...
   - Partial release-readiness rollup artifact: `artifacts/release/readiness/qaf035-release-readiness-20260602.json`.
     ...
     - `pnpm release:secret-hygiene:validate` is the current release-facing secret scan; it is expected to pass only when provider credentials remain in ignored local env/state, not committed evidence, release docs/templates, or current-truth handoff/backlog docs.
