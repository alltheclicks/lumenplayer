...
5. Release matrix guard added for beta readiness:
   ...
   - PR Quality Gate now runs `pnpm catchup:timeshift-hls:test`, so the historical local timeshift probe must keep exiting before ffmpeg/ffprobe probing.
   - `scripts/release/validate-beta-capacity-evidence.mjs` now gates the 300-500 user beta capacity artifact, explicitly requires `usesLocalFfmpeg=false`, `usesServerSideTranscode=false`, `usesServerSideRemux=false`, `usesGeneratedHls=false`, and `usesXuiSideTranscode=false`, and requires final media-path/no-media-processing evidence to cite both `release:no-media-evidence:scan` and a tracked redacted no-media scan-result artifact that validates.
   ...
   - `scripts/release/v1-manual-device-qa.template.json` and `scripts/release/validate-manual-device-qa.mjs` now define the exact real-device/manual QA evidence required for Windows Chrome, macOS Safari, Android Chrome, iOS Safari, Chromecast, AirPlay/Apple TV, and PWA install/offline; any passing target media audit must cite both `release:no-media-evidence:scan` and a tracked redacted no-media scan-result artifact that validates.
   - `artifacts/release/performance/qaf035-performance-evidence-20260603.json` and `scripts/release/validate-performance-evidence.mjs` now make performance readiness a concrete QAF artifact instead of a template-only reference; local benchmark commands and failed-run accounting validate now, while numeric startup/RSS values and owner approval remain pending for final signoff.
   - `scripts/release/v1-beta-ops-signoff.template.json` and `scripts/release/validate-beta-ops-signoff.mjs` now define the beta observability SLO, alert-route, rollback/throttle owner, enrollment stop, and no-media-processing stop-trigger evidence required before 300-500 live-user beta signoff; passing no-media ops signal/check evidence must cite `release:no-media-evidence:scan`, a tracked no-media scan artifact, and the runtime media policy artifact.
   - `artifacts/release/observability/qaf035-observability-baseline-20260603.json` and `scripts/release/validate-observability-baseline.mjs` now make observability a concrete release artifact instead of a loose unit-test reference: structured web event coverage and playback/cast alert rules validate now, while beta alert routing, dashboard, response SLO, and stop-trigger owners remain pending for final signoff.
   - `artifacts/release/security/qaf035-security-privacy-baseline-20260603.json` and `scripts/release/validate-security-privacy-baseline.mjs` now make security/privacy readiness a concrete QAF artifact: client storage inventory source refs, credential cleanup evidence, retention evidence, and secret hygiene coverage validate now, while incident owner/runbook/channel and final owner signoff remain pending.
   ...
   - `scripts/release/validate-release-readiness.mjs` now requires each pending gate in a triaged readiness artifact to be covered by an open/mitigated `blockerTriage.items[].gateIds` entry, so QAF readiness cannot show fewer active blockers than the final beta gate evidence actually requires.
   - `artifacts/release/readiness/qaf035-beta-closure-plan-20260603.json` and `scripts/release/validate-beta-closure-plan.mjs` now make the next-dev closure path concrete: every open blocker must map to the source readiness blocker, gate ids, existing artifact refs, required fields, validation commands, completion criteria, and the no-transcode/no-remux invariant.
   - `artifacts/release/readiness/qaf035-go-no-go-20260603.json`, `artifacts/release/smoke/qaf035-smoke-regression-matrix-20260603.json`, and `artifacts/release/design/qaf035-design-parity-20260603.json` now replace the remaining go/no-go, smoke/regression, and design-parity template refs in the concrete QAF readiness rollup while preserving pending final signoff.
   - `scripts/release/validate-go-no-go.mjs` and `scripts/release/validate-smoke-regression-matrix.mjs` now require passing catch-up no-media checks to cite `release:no-media-evidence:scan` and a tracked redacted no-media scan artifact that validates, so no-media go/no-go cannot pass from a plain prose evidence string.
   - `scripts/release/validate-design-parity-evidence.mjs` now requires existing rendered reference and Lumen capture files for every desktop/mobile screen in final mode, so design parity cannot pass with placeholder path strings.
   - The operator-facing package validators now target the concrete QAF-035 artifacts for readiness, provider owner, beta capacity, runtime media, manual-device QA, and beta ops instead of generic templates, so `pnpm release:readiness:validate` reports the current 9 open blockers until they are genuinely closed.
   - `scripts/release/validate-release-secret-hygiene.mjs` now scans release-facing evidence/docs/templates plus `HANDOFF.md`, `BACKLOG.md`, `VISION.md`, this V3 current-truth backlog, and `docs/catchup-timeshift-hls-evidence.md` for non-redacted provider credentials, credential query parameters, env secret assignments, and basic-auth URLs.
   - `scripts/release/validate-no-media-evidence-scan-artifact.mjs` is now required by the QAF aggregate gate and PR Quality Gate, so the tracked redacted Chrome Network/CDP scan-result artifact must keep raw captures untracked, source/report hashes recorded, forbidden-pattern coverage present, and media-processing hit counts at zero.
     - When the local raw Chrome Network/CDP files exist, the validator now recomputes their SHA256 values and fails if they differ from the tracked redacted artifact; it also checks those raw evidence refs are repo-relative, ignored by git, and not tracked by git.
   ...
   - Partial manual real-device QA artifact: `artifacts/release/manual-device-qa/qaf035-manual-device-qa-20260603.json`.
     - It is intentionally pending until each real target has an owner, actual device/browser version, screenshot/report evidence, network/no-media-processing evidence with tracked redacted scan-result artifact, and all release-blocker checks resolved.
   - Partial 300-500 user beta capacity artifact: `artifacts/release/capacity/qaf035-beta-capacity-20260602.json`.
     - `no-media-processing-verification` is pass from local browser/network/process evidence plus runtime media policy default no-remux checks and a tracked redacted no-media scan-result artifact.
   ...
   - Partial performance evidence artifact: `artifacts/release/performance/qaf035-performance-evidence-20260603.json`.
     - It validates local benchmark command coverage and `0/2` failed-run accounting for `pnpm perf:ttfc` and `pnpm perf:rss`.
     - It remains intentionally pending until numeric startup p95/p99, RSS peak/p95, and final release-owner performance signoff are recorded from an approved release run.
   ...
   - Partial beta ops signoff artifact: `artifacts/release/ops/qaf035-beta-ops-signoff-20260603.json`.
     ...
     - It explicitly treats local ffmpeg, server-side transcode/remux, generated HLS, proxy-remuxed/remux-hls, and XUI-side media processing as beta stop triggers, with final no-media ops evidence tied to the scanner and runtime media policy artifacts.
   - Partial observability baseline artifact: `artifacts/release/observability/qaf035-observability-baseline-20260603.json`.
     - It validates current source coverage for `playback.started`, `playback.error`, `playback.problem_reported`, `catchup.startup_timeout`, and `cast.error`, plus `alert.playback-error-burst` / `alert.cast-error-burst` threshold rules.
     - It remains intentionally pending until beta alert owner, channel, dashboard, response SLO, and stop-trigger owners are recorded.
   - Partial security/privacy baseline artifact: `artifacts/release/security/qaf035-security-privacy-baseline-20260603.json`.
     - It validates storage key source refs for credentials, app settings, theme, watch history, push metadata/device id, and XMLTV cache, plus passing control evidence for namespacing, logout credential cleanup, retention, and secret hygiene.
     - It remains intentionally pending until incident runbook, owner, response channel, and final security/privacy signoff are recorded.
   - Partial release-readiness rollup artifact: `artifacts/release/readiness/qaf035-release-readiness-20260602.json`.
     - It links current compatibility, capacity, provider owner, runtime media policy, performance evidence, observability baseline, security/privacy baseline, manual-device, and beta ops artifacts, records 9 explicit open blockers for go/no-go, target matrix, provider owner, beta capacity, performance, runtime no-media owner, observability, beta ops, and security/privacy signoff, and maps those blockers to every pending gate.
     - The matching closure plan `artifacts/release/readiness/qaf035-beta-closure-plan-20260603.json` records the exact owner placeholder, fields, artifact refs, commands, and no-media-safe completion criteria for each of those 9 blockers.
     ...
     - `pnpm release:secret-hygiene:validate` is the current release-facing secret scan; it is expected to pass only when provider credentials remain in ignored local env/state, not committed evidence, release docs/templates, current-truth handoff/backlog/vision docs, or historical provider-evidence docs.
