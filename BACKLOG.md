...
| QAF-024 | Normalize player scrollbar styling and fix TV-unazad overflow scrollbar artifacts | S | done | — |
| QAF-025 | Enforce live-edge restore on new-tab/session resume (avoid stale-segment black/static start) | M | done | QAF-015 |
| QAF-026 | Define and implement pause-resume stale policy for live playback (`resume` vs `snap-to-live`) | S | done | QAF-025 |
| QAF-027 | Add volume slider control in player overlay (desktop/mobile/PWA) | S | done | LP-0201 |
| QAF-028 | Fix Xtream series artwork mapping/loading (poster/banner parity with provider) | M | done | QAF-009 |
| QAF-029 | Improve catch-up blue bar seek affordance with handle/pointer + remote focus visibility | S | done | QAF-022 |
| QAF-030 | Fix catch-up seek playback failures (`Greška u mreži`) using provider-accepted timeshift URL/data path | M | done | QAF-018, QAF-022 |
...
| QAF-035 | Catch-up token path can return `200 video/mp2t` in web runtime without playable frame/audio; add playable-response gate and fallback continuation policy | M | pending-review | QAF-034 |
...
Completion notes (2026-02-19 .. 2026-02-21):
...
- QAF-024 merged via PR #198 (Greptile `5/5`).
- QAF-025 merged via PR #199 (Greptile `5/5`).
- QAF-026 merged via PR #200 (Greptile `4/5`; documented no-blocker exception with green local + CI gates).
- QAF-027 merged via PR #201 (Greptile `4/5`; documented no-blocker exception with green local + CI gates).
- QAF-028 merged via PR #203 (Greptile `5/5`).
- QAF-029 merged via PR #204 (Greptile `4/5`; documented no-blocker exception after final review pass with no valid blockers).
- QAF-030 merged via PR #205 (Greptile `4/5`; documented no-blocker exception pending provider runtime validation outside static review scope).
...
- QAF-035 delivered in PR #217 with local gates `PASS`; status is `pending-review` until PASS-100 closure (manual RTS1 runtime tuple unavailable in current env, Greptile PASS not available due subscription gate).
...
| LP-1511 | Build shared Xtream transport gateway (Fastify) for dev/beta/prod: same-origin media/API proxy, redirect-safe routing, timeout/retry policy, and unified observability | L | pending-review | LP-1507 |
