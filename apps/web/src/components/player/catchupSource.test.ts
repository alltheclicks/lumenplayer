...
const createShadowValidationUrlBuilder = () => ({
  getCatchUpRedirectUrlVariants: (
    ...
    `http://edge6.castcdn.net:8080/timeshift_hls/test-user/test-password/${durationSeconds}/${startTimestamp}/${streamId}.m3u8`,
  ],
  getCatchUpUrlVariants: (
    streamId: number,
    ...
    `http://smart.mediaking.fi:8080/streaming/timeshift.php?username=test-user&password=test-password&stream=${streamId}&start=2026-03-06:10-00&duration=30&extension=m3u8`,
  ],
  getLegacyCatchUpUrlVariants: (
    streamId: number,
    _startTimestamp: number,
    ...
    `http://smart.mediaking.fi:8080/timeshift/test-user/test-password/${durationSeconds}/2026-03-06:10-00/${streamId}.ts`,
  ],
});
...
describe('resolveCatchUpPlaybackSource', () => {
  ...
  it('uses a shadow-only transport plan when shadow validation resolves a tokenized edge manifest', async () => {
    ...
    expect(String(fetchImpl.mock.calls[0]?.[0])).toContain('/streaming/timeshift.php?username=test-user');
