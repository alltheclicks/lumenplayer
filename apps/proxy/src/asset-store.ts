...
export class AssetStore {
  ...
  set(record: AssetRecord): AssetRecord {
    this.assets.set(record.assetKey, record);
    return record;
  }
  ...
  markState(
    assetKey: string,
    state: CatchUpGatewayAssetState,
    ttlMs: number,
    options: {
      fallbackReason?: string | null;
      errorMessage?: string | null;
    ...
    const existing = this.get(assetKey, nowMs);
    if (!existing) {
      return null;
    }
    ...
    const nextRecord: AssetRecord = {
      ...existing,
      assetState: state,
      playbackUrl: options.playbackUrl ?? existing.playbackUrl,
      fallbackReason: options.fallbackReason ?? existing.fallbackReason,
