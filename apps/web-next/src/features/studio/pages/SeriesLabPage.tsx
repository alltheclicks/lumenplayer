...
import { CalendarDays, ChevronLeft, Clapperboard, Film, Home, Search, Tv } from 'lucide-react';
...
const getStatusBadgeClassName = (value?: string): string => {
  if (value?.toLowerCase().includes('zavrs')) {
...
const SeriesLabPage = () => {
  const { data } = useCatalogDataset('series');
      ...
      <div className="min-h-screen bg-background pb-24 md:pb-8">
        ...
        <main className="mx-auto max-w-[1440px] space-y-4 px-3 py-6 md:px-6">
          <div className="flex items-end justify-between gap-3">
            <h1 className="text-3xl font-semibold tracking-tight md:text-4xl">Sve serije</h1>
            <span className="text-sm text-muted-foreground">{filteredItems.length} serija</span>
          </div>
            ...
            {filteredItems.map((item) => (
              <article key={item.id} className="overflow-hidden rounded-2xl border border-border bg-card">
                <div className={cn('relative h-72 bg-gradient-to-b', item.gradientClassName)}>
                  ...
                  {item.statusLabel ? (
                    <div className={cn('absolute right-2 top-2 rounded-lg px-2.5 py-1 text-xs font-semibold', getStatusBadgeClassName(item.statusLabel))}>
                      {item.statusLabel}
                    </div>
                  ) : null}
