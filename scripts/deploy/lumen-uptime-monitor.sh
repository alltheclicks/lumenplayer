...
# - Secrets come from CONFIG_FILE (not from this script, which is in git).

set -uo pipefail

CONFIG_FILE="${LUMEN_MONITOR_CONFIG:-/etc/lumen/monitor.env}"
STATE_DIR="${LUMEN_MONITOR_STATE_DIR:-/var/lib/lumen-monitor}"
LOG_FILE="${LUMEN_MONITOR_LOG:-/var/log/lumen-monitor.log}"
