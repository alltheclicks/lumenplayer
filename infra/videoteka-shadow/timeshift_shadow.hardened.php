...
// SECURITY: token key must come from the environment, never be committed.
// Set SHADOW_TOKEN_KEY in the PHP-FPM pool env (or a non-repo include) on the
// server. The original live value is documented out-of-band, not in git.
define('SHADOW_TOKEN_KEY', getenv('SHADOW_TOKEN_KEY') !== false ? getenv('SHADOW_TOKEN_KEY') : '');
if (SHADOW_TOKEN_KEY === '') {
    header('HTTP/1.1 500 Internal Server Error');
    die('Shadow misconfigured: SHADOW_TOKEN_KEY not set.');
}
define('SHADOW_ARCHIVE_ROOT', IPTV_PANEL_DIR . 'tv_archive_shadow/');
define('SHADOW_CACHE_ROOT', '/tmp/catchup_shadow_hls/');

// --- Lumen hardening (2026-06-08) ---
// Conservative defaults; nothing here touches the original timeshift.php path
// or any non-Lumen client. All overridable via env without code edits.
define('SHADOW_MAX_CONCURRENT_BUILDS', getenv('SHADOW_MAX_CONCURRENT_BUILDS') !== false ? max(1, intval(getenv('SHADOW_MAX_CONCURRENT_BUILDS'))) : 4);
define('SHADOW_CACHE_GC_MAX_AGE', getenv('SHADOW_CACHE_GC_MAX_AGE') !== false ? intval(getenv('SHADOW_CACHE_GC_MAX_AGE')) : 7200);
define('SHADOW_CACHE_GC_PROBABILITY', getenv('SHADOW_CACHE_GC_PROBABILITY') !== false ? intval(getenv('SHADOW_CACHE_GC_PROBABILITY')) : 25); // 1-in-N requests runs an inline sweep
// Dynamic codec map: a cron (probe-codec-map.sh) refreshes this JSON with the
// real per-stream audio codec. Channels can flip MP2<->AAC at the source, so
// this is a LIVE signal, not a hardcoded whitelist. If a stream is known to be
// browser-safe (AAC/MP3), shadow steps aside and the normal path serves it,
// avoiding pointless transcode. Stale or missing map => fail-open (still probe).
define('SHADOW_CODEC_MAP_FILE', getenv('SHADOW_CODEC_MAP_FILE') !== false ? getenv('SHADOW_CODEC_MAP_FILE') : '/tmp/catchup_shadow_codecmap.json');
define('SHADOW_CODEC_MAP_MAX_AGE', getenv('SHADOW_CODEC_MAP_MAX_AGE') !== false ? intval(getenv('SHADOW_CODEC_MAP_MAX_AGE')) : 6 * 3600);
...
// --- Lumen hardening helpers ---

// Recursively delete a cache window directory (best-effort).
function shadowRemoveTree($dir)
{
    if (!is_dir($dir)) {
        @unlink($dir);
        return;
...
// Inline garbage collector: drops cache windows whose playlist (or the dir
// itself) hasn't been touched within SHADOW_CACHE_GC_MAX_AGE. Runs only on a
// 1-in-N sampled request so it never blocks the hot path. Safe even if /tmp is
// large — this just bounds growth over time.
function shadowMaybeGarbageCollect()
{
    if (SHADOW_CACHE_GC_PROBABILITY > 1 && (mt_rand(1, SHADOW_CACHE_GC_PROBABILITY) !== 1)) {
        return;
    }
    $now = time();
    foreach (glob(rtrim(SHADOW_CACHE_ROOT, '/') . '/*', GLOB_ONLYDIR) as $dir) {
        $playlist = $dir . '/playlist.m3u8';
        $ref = file_exists($playlist) ? filemtime($playlist) : filemtime($dir);
        if ($ref !== false && ($now - $ref) > SHADOW_CACHE_GC_MAX_AGE) {
            // Never evict a window that is actively building.
            $lock = $dir . '/remux.lock';
            if (file_exists($lock) && ($now - filemtime($lock)) < 300) {
                continue;
            }
...
// Global concurrency guard. We count cache windows whose remux.lock is fresh
// (a build in progress). Above the cap we refuse to START a new build and tell
// the client to retry — preventing an ffmpeg stampede under load. This is a
// global cap across all streams, complementing the existing per-window lock.
function shadowActiveBuildCount()
{
    $now = time();
    $count = 0;
    foreach (glob(rtrim(SHADOW_CACHE_ROOT, '/') . '/*/remux.lock') as $lock) {
...
// Dynamic codec map lookup. Returns:
//   'safe'    -> stream is browser-playable audio (AAC/MP3); shadow can step aside
//   'unsafe'  -> stream needs transcode (MP2/AC3/etc.)
//   'unknown' -> no fresh map entry; caller should fall back to live ffprobe
function shadowCodecMapStatus($streamId)
{
    if (!file_exists(SHADOW_CODEC_MAP_FILE)) {
        return 'unknown';
    }
    if ((time() - filemtime(SHADOW_CODEC_MAP_FILE)) > SHADOW_CODEC_MAP_MAX_AGE) {
        return 'unknown'; // stale map -> fail-open to probing
    }
    $map = @json_decode(@file_get_contents(SHADOW_CODEC_MAP_FILE), true);
    if (!is_array($map) || !isset($map['streams']) || !isset($map['streams'][(string) $streamId])) {
        return 'unknown';
    }
    $entry = $map['streams'][(string) $streamId];
    $audio = isset($entry['audio_codec']) ? strtolower($entry['audio_codec']) : '';
    if ($audio === '') {
        return 'unknown';
    }
    // Browsers decode AAC and MP3 in MSE; everything else (mp2, ac3, eac3...) needs transcode.
    return in_array($audio, array('aac', 'mp3'), true) ? 'safe' : 'unsafe';
}
...
// Dynamic step-aside: if the cron-maintained codec map says this stream is
// already browser-playable (AAC/MP3), there's no reason to transcode. Tell the
// Lumen client to use the normal catch-up path. We do this ONLY for the
// manifest request (segment requests must still be served from cache), and
// 'unknown'/stale always falls through to a real ffprobe — so a channel that
// silently flips to MP2 is still caught on the next build.
if ($request['segment_index'] === null && shadowCodecMapStatus($shadowStreamId) === 'safe') {
    shadowLog('shadow step-aside stream=' . $shadowStreamId . ' reason=codecmap_safe');
    header('HTTP/1.1 409 Conflict');
    header('X-Lumen-Shadow: step-aside-safe');
    die;
}
...
// Bound /tmp growth (sampled, non-blocking).
...
if ($needsBuild) {
    if (file_exists($lockFile) && (time() - filemtime($lockFile)) < 300) {
        ...
        // Global concurrency cap: refuse to START a new build when too many are
        // already running, so a traffic spike cannot spawn unbounded ffmpeg.
        // A serve-from-cache request (no build needed) never reaches here.
        if (shadowActiveBuildCount() >= SHADOW_MAX_CONCURRENT_BUILDS) {
            shadowLog('shadow throttle stream=' . $shadowStreamId . ' active=' . shadowActiveBuildCount() . ' cap=' . SHADOW_MAX_CONCURRENT_BUILDS);
